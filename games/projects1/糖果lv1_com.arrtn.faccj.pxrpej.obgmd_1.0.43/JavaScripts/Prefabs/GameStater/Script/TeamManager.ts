...
export namespace TeamManager {
    ...
    */
    ...
    //     return true;
    // }
    async function getTeamData(teamdId: string) {
        let maxTry = 10;
        return new Promise((res) => {
            const inter = setInterval(() => {
                const data = RouteService.getTeamCarryingData(teamdId);
                maxTry--;
                if (data || maxTry <= 0) {
                    clearInterval(inter);
                    res(data);
    ...
    export async function addPlayer(userId: string, teamId: string, player: mw.Player) {
        let data = null;
        if (SystemUtil.isServer() && !TestMode.testMode) {
            data = await getTeamData(teamId);
            if (!data) {
                Event.dispatchToClient(player, "TeamId.Undefined", NO_TEAMDATA);
                return null;
            }
        }
        let team = teams.find(i => i.teamId == teamId);
        if (!team) {
            team = new Team(teamId, data, config);
            teams.push(team);
            team.start();
        } else {
            // team.reset();
    ...
    export function init(_config: GameDataConfig) {
        ...
        if (SystemUtil.isServer()) {
            ...
            Event.addClientListener("Player.Join", (player, userId: string, teamId: string) => {
                addPlayer(userId, teamId, player);
            });
            ...
            mw.Player.asyncGetLocalPlayer().then(player => {
                const serverInter = setInterval(() => {
                    if (player.userId && player.teamId) {
                        ...
                        Event.dispatchToServer("Player.Join", player.userId, player.teamId);
                    }
                }, 3000);
                let maxRetry = 50;
                const inter = setInterval(() => {
                    if (player.userId && player.teamId) {
                        addPlayer(player.userId, player.teamId, player);
                        clearInterval(inter);
                    } else if (maxRetry-- <= 0) {
    ...
    Event.addLocalListener("GameState.ResGamingCharacter", (data: any) => {
        ...
        for (const team of teams) {
            team.clientPlayerCount = data.playerCount;
