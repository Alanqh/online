 ...
 * @LastEditTime : 2023-10-20 15:19:29
 * @FilePath     : \stumbleguys\JavaScripts\modules\ProcessModule\ProcessModuleC.ts
* @Description  : 流程模块客户端
*/
...
export default class ProcessModuleC extends ModuleC<ProcessModuleS, null> {
    ...
    protected onStart(): void {
        ...
        Event.addLocalListener("GameState.End.Client", () => {
            ...
            if (round == 2) {
                ...
                if (!this._isPass && this._passByTeamId == 0) {
                    if (!isEliminate) {
                        this._resultUIs.push({ ui: OutUI, isShow: false });
                        mw.UIService.show(OutUI);
                    }
                } else {
        ...
        Event.addLocalListener("ShiftResultUI", (uiClass: new () => mw.UIScript) => {
            if (this._resultUIs.length > 0 && uiClass === this._resultUIs[0].ui) {
    ...
    protected onUpdate(dt: number): void {
        ...
        if (this._resultUIs.length > 0) {
            ...
            if (resultUI && !resultUI.isShow) {
                resultUI.isShow = true;
                mw.UIService.show(resultUI.ui);
            }
        }
    ...
    private async showAIModel() {
        this.showCLAI();
        const teamId = this.localPlayer.teamId;
        const aiDatas = TeamManager.getTeam(teamId).clientAIData;
    ...
    private showPassUI(guid: string, isEnd: boolean): void {
        ...
        if (curGuid == guid && !mw.UIService.getUI(PassUI, false)) {
            this._resultUIs.push({ ui: PassUI, isShow: true });
            mw.UIService.show(PassUI, isEnd ? this._passByTeamId : 0, passTime);
    ...
    private showeEndUI(): void {
        // this._resultUIs.push({ ui: EndUI, isShow: false });
        const round = this._curGameInfo.round;
        if (round == 2) {
            // 获取排名
            Singleton.getIns(MGSCore).coreStep(13);
    ...
    public net_enterLobby(lobbyId: string, fristRank: number, finalRank: number, isAfk: boolean, exGold: number, boxPassword: BoxPassword): void {
        const data = { fristRank: null, finalRank: null, isAfk: null, exGold: 0, boxPassword: null };
        ...
        // 发起跳游戏
        ProcessHelper.enterGameClient(lobbyId, data);
    }
    ...
    public net_updatePlayerNum(pNum: number, seat: number, name: string, maxCount: number): void {
        ...
        const teamID = this.localPlayer.teamId;
        if (teamID != null) {
            const teamData = TeamManager.getTeam(teamID)
    ...
    public getTeamPlayer() {
        const teamId = this.localPlayer.teamId;
        const players = TeamManager.getTeam(teamId) ? TeamManager.getTeam(teamId).players : [];
        return players
    }
