...
export class PlayerManagerExtesion {
    ...
    public static loadAnimationExtesion(char: mw.Character, assetid: string, sync?: boolean) {
        sync = sync === undefined ? true : sync;
        let isChar = char instanceof mw.Character;
        if (!isChar) return;
        if (!this.isUseRpc(sync)) {
...
class RpcExtesionC extends ModuleC<RpcExtesionS, null>{
    ...
    public async net_playAnimation(charGuid: string, assetId: string, rate: number, loop: number, slot: mw.AnimSlot) {
        if (this.localPlayer.character == null || charGuid == this.localPlayer.character.gameObjectId) return;
