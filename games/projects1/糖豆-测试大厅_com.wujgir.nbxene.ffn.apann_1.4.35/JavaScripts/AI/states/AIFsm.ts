...
export class AIFsm {
    ...
    public walkToSafe() {
        let cells = OverturnClient.instance.cells.filter(cell => { return !cell.isTurn && cell.id != 0 });
        if (cells.length > 0) {
            let cell = Utils.randomInList(cells)
            this.walkTo(cell.gameObject.worldTransform.position, () => {
                this.changeState(WalkSafeState, cell.id)
            })
        } else {
...
class WalkSafeState implements IState {
    ...
    onUpdate(dt: number) {
        this.time -= dt;
        if (this.cell.isTurn || this.time < 0) {
            this.fsm.walkToSafe();
        }
    }
    onEnter(id: number) {
        this.time = Utils.rangeInt(2, 5);
        this.cell = OverturnClient.instance.cells[id];
    }
