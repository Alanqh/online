...
class ClientAIService implements IAIService {
    ...
    public init(maxJumpHeight: number, maxWalkSpeed: number, gravityScale: number) {
        ...
        Event.addLocalListener("AI.MoveToPoint", (character: mw.Character, location: mw.Vector) => {
            const robot = this.robots.find(i => i.character == character);
            if (robot) {
                robot.moveToPoint(location);
            }
        });
        Event.addLocalListener("AI.MoveToObject", (character: mw.Character, object: mw.GameObject, callback: () => void, success?: (npcLocation: Vector, objectLocation: Vector) => boolean) => {
            const robot = this.robots.find(i => i.character == character);
            if (robot) {
                robot.moveToObject(object, callback, success);
            }
        });
