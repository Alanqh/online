...
import { OverturnClient } from "./rule/OverturnClient";
import { OverturnRule } from "./rule/OverturnRule";
...
export default class GameStart extends mw.Script {
    ...
    private jumpForce = new mw.Vector(500, 0, 0);
    protected onStart() {
        ...
        if (SystemUtil.isClient()) {
            ...
            Player.asyncGetLocalPlayer().then(async (player) => {
                ...
                PlayerCtrlMgr.ins.actionFSM.character.beforeJump = (character: mw.Character) => {
                    if (character.worldTransform.position.z > 600) {
                        character.addImpulse(this.jumpForce, true);
                    }
                }
                this.useUpdate = true;
            });
            ...
            //   });
            // }
            Event.addLocalListener("Team.Create", (team: Team) => {
                team.rules.push(new OverturnRule(team));
            });
        }
        if (this._debugScene) {
            if (SystemUtil.isClient()) {
    ...
    protected onUpdate(DeltaTime: number): void {
        if (this.isRunningClient()) {
            ...
            OverturnClient.instance.onUpdate(DeltaTime);
        }
    }
}