...
export class PlayerManagerExtesion {
    ...
    public static isNpc(obj: GameObject): obj is Character {
    ...
    private static isUseRpc(isSync: boolean, char: Character): boolean {
    ...
    public static stopStanceExtesion(char: Character, sync?: boolean): void {
        sync = sync === undefined ? true : sync;
        if (!this.isUseRpc(sync, char)) {
    ...
    public static changeBaseStanceExtesion(char: Character, assetId: string): void {
        if (!this.isUseRpc(true, char)) {
            if (assetId == "") {
    ...
    public static changeStanceExtesion(char: Character, assetId: string): void {
        let sync = true;
        if (!this.isUseRpc(sync, char)) {
            if (assetId == "") {
                char.currentSubStance?.stop();
    ...
    public static loadStanceExtesion(char: Character, assetId: string, sync?: boolean): mw.SubStance {
        sync = sync === undefined ? true : sync;
        if (!this.isUseRpc(sync, char)) {
    ...
    public static rpcPlayAnimation(owner: Character, assetId: string, loop: number = 1, speed: number = 1) {
    ...
    public static rpcStopAnimation(owner: Character, assetId: string): void {
        if (owner.currentAnimation && owner.currentAnimation.assetId == assetId) {
            owner.currentAnimation.stop();
        }
        if (this.isUseRpc(true, owner)) {
            let module = ModuleService.getModule(RpcExtesionC);
    ...
    public static rpcPlayAnimationLocally(owner: Character, assetId: string, AnimationLength: number = 0, loopCount: number = 1) {
    ...
    public static loadAnimationExtesion(char: Character, assetid: string, sync?: boolean) {
        sync = sync === undefined ? true : sync;
        if (!this.isUseRpc(sync, char)) {
...
class RpcExtesionS extends ModuleS<RpcExtesionC, null>{
    ...
    public async net_playBasicStance(charGuid: string, basicStance: string) {
        let char = await GameObject.asyncFindGameObjectById(charGuid) as Character;
        if (!basicStance) {
            if (char.currentStance) char.currentStance.stop()
        } else {
