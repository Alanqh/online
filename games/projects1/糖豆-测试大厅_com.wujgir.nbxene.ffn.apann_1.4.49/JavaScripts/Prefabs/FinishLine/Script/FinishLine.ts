 ...
 * @LastEditTime : 2023-12-13 14:40:28
        ...
        if (mw.SystemUtil.isServer()) {
            ...
            (this.gameObject as mw.Trigger).onEnter.add((character) => {
                if (PlayerManagerExtesion.isCharacter(character)) {
                    ...
                    if (character.player["leftEffect"] != undefined) EffectService.stop(character.player["leftEffect"])
                    if (character.player["rightEffect"] != undefined) EffectService.stop(character.player["rightEffect"])
                }
