 ...
 * @LastEditTime : 2023-12-21 14:02:25
 * @FilePath     : \stumbleguys_new\JavaScripts\modules\dress\ItemObj.ts
* @Descripttion : 
*/
...
export default class ItemObj extends TagScript {
	...
	@mw.Property({ replicated: true, onChanged: "onIceCubeChanged" })
	private _iceCube: boolean = false;
	...
	override onStart() {
		...
		if (mw.SystemUtil.isClient()) {
			...
			Event.addLocalListener("Hand.Hide", (character: mw.Character, status: PropertyStatus) => {
				console.log("Hand.Hide", character, status);
			...
			GameObject.asyncSpawn("6D9B8FFA4F49CEA56A96E0B3B6BADE6A").then((obj) => {
				...
				let interval = setInterval(() => {
					if (this.player && this.player.character) {
						this.player.character.attachToSlot(obj, HumanoidSlotType.Root)
	...
	/**被冻住 */
	@RemoteFunction(mw.Server)
	beFrozen(player: Player) {
		player.character.setCollision(CollisionStatus.QueryOnly)
		this._iceCube = true
	}

	/**解冻 */
	@RemoteFunction(mw.Server)
	unFrozen(player: Player) {
	...
	onIceCubeChanged() {
		if (this._iceCubeObj) {
			if (this._iceCube) {
				this._iceCubeObj.setVisibility(true)
			} else {
				this._iceCubeObj.setVisibility(false)
			}
		}
