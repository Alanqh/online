...
abstract class WeaponManagerCli extends WeaponManagerPreDefine {
     ...
     * @returns 
     */
    protected async onServerInitWeaponCompalte(netActionId: number, objGuid: string, charGuid: string) {
        if (this.netEventRet.has(netActionId)) {
            let obj = await GameObject.asyncFindGameObjectById(objGuid)
            let script = obj.getComponent(Weapon);
...
class WeaponManagerSvr extends WeaponManagerCli {
    ...
    public async client_AsyncGetWeapon(char: mw.Character | mw.Character, reqPlayer: mw.Player, netActionId: number): Promise<Weapon> {
        ...
        if (mw.SystemUtil.isServer()) {
            ...
            let script = test.addComponent(Weapon);
            ...
            testWeapon.initCharacter(char, (() => {
                ...
                Event.dispatchToClient(reqPlayer, this.onServerInitWeaponCompalte.name, netActionId, test.gameObjectId, char.gameObjectId);
            }).bind(this));
            return null;
