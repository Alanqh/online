...
export abstract class WeaponClient extends WeaponData {
    ...
    private _onSendFlyItem: (info: NodeInfoFlyEntity) => void = null;
    ...
    protected onDestroy(): void {
        ...
        this._cacheFlyNode = {};
        if (this._char && this._char.worldTransform) {
            console.log("rock onDestroy");
            this._char.movementEnabled = true;
            this._char.jumpEnabled = true;
        }
        this._stance?.stop();
        this.stopAllTimer();
        super.onDestroy();
    ...
    protected async reWriteAnimationJson(skillIds: number[]) {
        ...
        if (!WeaponData.getWeaponJson) {
            ...
            jsonList = WeaponData.getWeaponJson(...skillIds);
        }
        this.animationJsons = [];
    ...
    public async playAnimation(index: number,
        ...
        onChangeControl: (canControl: boolean) => void,
        onSendFlyItem: (info: NodeInfoFlyEntity) => void
    ): Promise<boolean> {
        ...
        this._onSendFlyItem = onSendFlyItem;
        ...
        /** 处理战斗待机 */
        this._fightingSec = 0;
        this._char.movementEnabled = false;
        this._char.jumpEnabled = false;
        //设置前摇时间，丢失控制
        ...
        this.startTimeOut(() => {
            ...
            if (periodInfo.isAutoPlay != "0") {
                this.playAnimation(this._curAnimationIndex, this._onComplate, this._onHit, this._onStartCombo, this._onStartCharge, this._onChangeControl, this._onSendFlyItem);
                return;
            }
        }, logicFrontTime);
    ...
    private startPeriod(animationIndex: number, periodIndex: number, isSelf: boolean = false) {
        ...
        if (isSelf) {
            ...
            if (SystemUtil.isClient() && PlayerManagerExtension.isCharacter(this._char)) {
            ...
            if (this._onHit) {
                skillRectNodes.forEach(e => {
                    ...
                    if (e.delayPlayTime == "" || isNaN(tempTime)) {
                        this._onHit(e.skillIndex, this._maxIndex, SkillRectCheck.checkNodes(this._char, skillRectCheck));
                        return;
                    }
                    setTimeout(() => {
                        this._onHit(e.skillIndex, this._maxIndex, SkillRectCheck.checkNodes(this._char, skillRectCheck));
    ...
    public stopPlay() {
        ...
        this._onComplate && this._onComplate(0, 0);
        if (this._stance) this._stance.stop();
        //this._lastPlayGuid = "";

    }

    private _lerpTargets: string[] = [];
    ...
    * @param time 
    */
    protected async characterLerpMove(guid: string, startPos: mw.Vector, pos: mw.Vector, time: number, moveType: number, movedis: number, dmgRate: string, radius: Vector2) {
        ...
        if (go instanceof mw.Pawn) {
            ...
            this.startUpdateInterval((dt: number): boolean => {
                ...
                res = res.filter(e => { return !(e.gameObject instanceof mw.Character) && !(e.gameObject instanceof mw.Trigger) && !(e.gameObject instanceof UIWidget) });
                if (moveType == 2) {
                    if (!lastCheckPos) {
                        lastCheckPos = curVec;
                    }
                        ...
                        if (curFrame < 5) {
                            ...
                            curFrame = 0;
                            let hitRes = SkillRectCheck.modifyboxOverlapInLevel(lastCheckPos, curVec, radius.x, radius.y, SkillRectCheck.showRect, this._lerpTargets);
                            lastCheckPos = curVec;
                            this._lerpTargets.push(...hitRes.map(e => { return e.gameObjectId }));
                            hitRes.length != 0 && this._onHit(dmgRate, this._maxIndex, hitRes);
                        }
                    }
                }
    ...
    private setCharAutoFocus(animationIndex: number) {
        ...
        let res = SkillRectCheck.checkRadius(this._char, this._char.worldTransform.position, radius, 150);
        res = SkillRectCheck.checkAngle(this._char.worldTransform.position, this._char.worldTransform.getForwardVector(), parseInt(aniInfo.autoFocusAngle), res);
        res = res.filter(e => e.gameObjectId != this._char.gameObjectId).filter(WeaponData.focusFilter);
        if (res.length <= 0) {
            return;
        }
        console.log("setCharAutoFocus res length", res.length);
        const forward = this._char.worldTransform.getForwardVector().clone();
        const curloc = this._char.worldTransform.position.clone();
        ...
        if (candidate) {
            setTimeout(() => {
                this._char.lookAt(candidate.worldTransform.position);
            }, 1);
        }
    }
        ...
        let handle = setInterval(async () => {
            ...
            if (char) {
                ...
                if (PlayerManagerExtension.isCharacter(char)) {
                    this._char = char as mw.Character;
                } else {
                    this._char = char as mw.Character;
                }
            }
        ...
        if (this.equipRightWeaponGuid != "") {
            const handle = setInterval(async () => {
                if (this._char != null) {
                    ...
                    this._char.attachToSlot(this._clientEquipGo_Right, mw.HumanoidSlotType.RightHand);
        ...
        if (this.equipLeftWeaponGuid != "") {
            const handle = setInterval(async () => {
                if (this._char != null) {
                    ...
                    this._char.attachToSlot(this._clientEquipGo_Left, mw.HumanoidSlotType.LeftHand);
    ...
    */
    protected async client_PlayEffect(guid: string, stopTime: number, loop: boolean, slotIndex: number,
        ...
        if (slotIndex == -1) {
            ...
            this._char.attachToSlot(eff, slotIndex);
    ...
    private startFireNode(fireNode: NodeInfoFlyEntity[], isSelf: boolean) {
        fireNode.forEach(info => {
            const playInfo = info;
            this.startTimeOut(() => {
                this._onSendFlyItem && this._onSendFlyItem(info);
        ...
        if (go) {
            ...
            const handle = setInterval(() => {
                ...
                if (isSelf) {
                    ...
                    res = res.filter(e => {
                        return (e instanceof mw.Pawn) && e.gameObject != go && e.gameObject.gameObjectId != this._char.gameObjectId
                    })
                    if (res.length > 0) {
    ...
    startThrowNode(followNode: NodeInfoThrow[], isSelf: boolean) {
        followNode.forEach(info => {
            ...
            this.startTimeOut(() => {
                const rotation = new mw.Rotation(
                    parseInt(playInfo.offsetRotation[0]), parseInt(playInfo.offsetRotation[1]), parseInt(playInfo.offsetRotation[2]));
                let dir = rotation.rotateVector(this._char.worldTransform.getForwardVector().normalize());
                let startPos = this._char.worldTransform.position.clone();
            ...
            throwGo.intervalHandle = setInterval(() => {
                ...
                if (isSelf) {
                    ...
                    if (throwGo.checkTimer <= 0) {
                        ...
                        let res = SkillRectCheck.checkRadius(this._char, throwGo.go.worldTransform.position, skillRadius, 200);
                        res = res.filter(e => { return e != throwGo.go && e.gameObjectId != this._char.gameObjectId })
                        if (res.length > 0) {
    ...
    private moveHandle(moveInfo: NodeInfoMove[]) {
        ...
        moveInfo.forEach(e => {
            ...
            let dmgRadius = new Vector2(parseFloat(e.dmgRadius[0]), parseFloat(e.dmgRadius[1]));
            this.startTimeOut(() => {

                if (moveRot) {
                    this.characterLerpRot(this._char.gameObjectId, this._char.localTransform.rotation, moveRot, dur);
                }
                /** 处理瞬移 */
                if (moveType == 1) {
                    let charPos = this._char.worldTransform.position;
                    let toPos = charPos;
                    let curMoveDis: Vector = null;
                    let offset = this._char.worldTransform.getForwardVector().toRotation();
                    ...
                    while (true) {
                        ...
                        if (res.findIndex(e => { return e.gameObject == this._char }) != -1 && res.length == 1) {
                            break;
                        }
                        if (res.length > 0) {
                            rate -= 0.2;
                    ...
                    this._char.worldTransform.position = (toPos);
                    return;
                }

                //非瞬移
                let charPos = this._char.worldTransform.position.clone();
                let toPos = charPos;
                let curMoveDis: Vector = null;
                let offset = this._char.worldTransform.getForwardVector().toRotation();
                curMoveDis = offset.rotateVector(moveDirAll).multiply(moveDis);
                toPos = toPos.clone().add(curMoveDis);
                this.characterLerpMove(this._char.gameObjectId, charPos, toPos, dur, moveType, moveDis, e.dmgRate, dmgRadius);
            }, moveDelay);
        })
    }
    ...
    private animasHandle(animInfos: NodeInfoAnim[]) {
        animInfos.forEach(info => {
            ...
            this.startTimeOut(() => {
                ...
                //     this._lastAnimation = null;
                // }
                const anim = PlayerManagerExtension.loadAnimationExtesion(this._char, info.guid)
    ...
    private playAnimationEff(effs: NodeInfoEff[]) {
        ...
        effs.forEach(e => {
            ...
            setTimeout(async () => {
                ...
                if (slotIndex <= -1) {
                    offsetRotate = this._char.worldTransform.getForwardVector().toRotation().clone().add(new mw.Rotation(offsetRotate.x,
                        offsetRotate.y, offsetRotate.z));
                    let forward = this._char.worldTransform.getForwardVector().normalized;
                    let right = this._char.worldTransform.getRightVector().normalized;
                    offsetPos = this._char.worldTransform.position.clone().add(
                    ...
                    if (slotIndex == -2) {
                        offsetPos = offsetPos.clone().subtract(mw.Vector.up.multiply(this._char.collisionExtent.z / 2));
                    }
                    if (this._syncEff) {
                    ...
                    if (this._char.player == null) return;
                    if (this._syncEff) {
                        this.client_PlayEffect(e.guid, parseInt(e.stopTime), parseInt(e.loop) == 1, slotIndex, offsetPos, offsetRotate, offsetScale, e.colorHex);
                    }
                }
    ...
    onUpdate(dt: number) {
        ...
        if (mw.SystemUtil.isClient()) {
            ...
            if (this._canOper && this._canCombo) {
                /** 角色在移动 */
                if (this._char.isMoving && (this._lastPlayGuid != "" || this._fightIdelAniGuid != "")) {
            ...
            else if (this._char != null && this._fightIdelAniGuid != "") {
                if (this._fightingSec <= 0 || (this._char.isMoving && !this._char.isJumping)) {
                    if (this._lastPlayGuid == this._fightIdelAniGuid) {
                        console.log("moving:", this._char.isMoving + "jumping:" + this._char.isJumping + "fightSecond:" + this._fightingSec)
                        if (this._lastAnimation && this._lastAnimation.slot != 1) {
