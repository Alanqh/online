 ...
 * @LastEditors  : dal
 * @LastEditTime : 2024-03-13 13:35:21
 * @FilePath     : \hauntedparadise\JavaScripts\codes\utils\TraceHelper.ts
 * @Description  : 
 */
export namespace GhostTraceHelper {
    ...
    /** 背包操作 */
    export function rebirthNum(wepaonId: number, num: number) {
        let desc = "背包操作_";
        switch (num) {
            case 0:
                desc += "点击物品展开tips";
        ...
        this.uploadMGS("ts_action_buy_weapon", desc, { weapon_id: wepaonId, rebirth_num: num });
    }
    ...
    */
     ...
     * @param source 获得的来源 ， 0为商店白嫖，1为商店购买，2为宝箱获取，3为场景内拾取，4为UI上拾取（两个地方，一个是伪人，一个是结局哪里。）
     */
    export function onGetItem(itemid: number, source: number) {
        this.uploadMGS("ts_action_upgrade_item", "获得物品的时候上发", { item_id: itemid, item_level: source });
    }


    export function onJumpFindGuide(guideStage: number, id: number) {
        GhostTraceHelper.uploadMGS("ts_game_over", "Find引导步骤", {
            round_id: 901,
            point_hold: id,
            polong_hold: guideStage
        })
    }
} 