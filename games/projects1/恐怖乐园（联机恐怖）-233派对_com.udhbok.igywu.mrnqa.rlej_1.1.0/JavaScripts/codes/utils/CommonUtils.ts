 ...
 * @LastEditTime : 2024-06-16 16:27:06
 * @FilePath     : \1004_graveyard\JavaScripts\codes\utils\CommonUtils.ts
 * @Description  : 
 */
...
export class CommonUtils {
    ...
    /** 用于配置表上的转换 */
    public static vec2Rot(vec: Vector) {
    ...
    public static formatTimeCH(min: number) {
        // 转换分钟为秒  
        ...
        // 格式化输出字符串，只有当值不为0时才显示  
        let formatted = [];
        if (days > 0) formatted.push(days + '天');
        if (hours > 0) formatted.push(hours + '小时');
        if (minutes > 0) formatted.push(minutes + '分');
        if (seconds > 0 || formatted.length === 0) formatted.push(seconds.toFixed(0) + '秒'); // 至少显示秒  
        // 返回格式化后的字符串  
    ...
    public static roundVec(vec: Vector, num: number) {
        const multiplier = Math.pow(10, num);
        vec.x = (vec.x * multiplier | 0) / multiplier
        vec.y = (vec.y * multiplier | 0) / multiplier
        vec.z = (vec.z * multiplier | 0) / multiplier
    }

    public static splitName(str: string) {
        //英文括号
