...
export default class MusicMgr {
    ...
    private play3DSound(guid: string, target: string | GameObject | Vector, volume: number = 1, loopNum: number = 1, radius: number) {
        if (!target) {
            // console.error(`3D音效${guid}没填宿主，将默认在Player身上播放`);
            this.play3DSound(guid, Player.localPlayer.character, volume, loopNum, radius);
        }
        return SoundService.play3DSound(guid, target, loopNum, volume, { radius: 1e3, falloffDistance: radius });
    }
    ...
    async play(soundId: number, target?: string | GameObject | Vector) {
        ...
        switch (soundCfg.type) {
            ...
            case 1:
                ...
                break;
            case 2:
                this.soundMap.set(soundCfg.guid, this.play3DSound(soundCfg.guid, target, soundCfg.volume, soundCfg.loopNum, soundCfg["radius"] || 3e3));
                break;
            default:
