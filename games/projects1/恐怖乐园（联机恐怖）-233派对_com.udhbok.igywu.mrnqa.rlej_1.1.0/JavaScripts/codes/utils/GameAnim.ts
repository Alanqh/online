...
export class GameAnim {
    ...
    public static async flySequence(count: number, start: mw.Widget | mw.Vector, tag: mw.Image | mw.Widget, imgIcon?: string, size?: mw.Vector2, ofst?: mw.Vector2, over?: () => void, rang = 100) {
        ...
        overPos.x += tag.size.x / 2
        overPos.y += tag.size.y / 2
        ofst && startPos.add(ofst);

        // 创建UI
        if (!this.flyRoot) {
            // const screen = mw.WindowUtil.getViewportSize();
            const root = this.flyRoot = UIService.getUI(Animation2D_Generate).uiWidgetBase//mw.UserWidget.newObject();
            // root.size = screen;
        ...
        // this.flyRoot.zOrder = mw.UILayerSystem
        // this.flyRoot.invalidateLayoutAndVolatility()
        // 创建图片
