...
export class GhostDistanceUI extends GhostDistance_UI_Generate {
    ...
    private static _cacheUIs: GhostDistanceUI[] = [];

    public static setTargetLoc(target: Character, bindIns: GhostBehavoirInst) {
        let ui = GhostDistanceUI._cacheUIs.pop();
        if (!ui) {
            ui = UIService.create(GhostDistanceUI);
        }
        ui.setTargetLoc(target, bindIns);
    }

    public static despawn(ui: GhostDistanceUI) {
        this._cacheUIs.push(ui);
    }
