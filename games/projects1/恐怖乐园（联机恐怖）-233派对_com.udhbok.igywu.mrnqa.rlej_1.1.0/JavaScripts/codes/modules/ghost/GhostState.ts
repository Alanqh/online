...
export class GhostStateChase extends GhostStateBase {
    private _ghostModuleS: GhostModuleS

    enter() {
        console.log("enter chase");

        if (!this._ghostModuleS) {
            this._ghostModuleS = ModuleService.getModule(GhostModuleS);
        }
    ...
    update(dt: number): void {
        ...
        if (!this._ghostModuleS.checkCanKill(this.owner.targetId)) {
            this.owner.server_exitChase();
        }
        super.update(dt);
    }
