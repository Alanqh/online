...
export class GhostMoveProxy {
    ...
    onUpdate(char: Character, rot: Rotation) {
        ...
        if (this.isFly) {
            this._tempRot.set(rot);
            this._tempRot.x = this._tempRot.y = 0;
            char.worldTransform.rotation = this._tempRot;
            const force = rot.getForce(this._tempVec);
            char.addMovement(force)
            this.failFunc(char);
        }
        else {
