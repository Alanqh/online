...
export default class ProcedureScript extends PlayerState {
    ...
    public startIntervalSave() {
        this.server_saveTimer.setInterval(async () => {
            console.log("DEBUG>>> 触发定时存档机制");
            if (!(await ArchiveHelper.reqGetData(this.userId, this.archiveID)).isInit) { return; }
    ...
    public async countUseTime() {
        ...
        if (this.endType === EndType.Success) {
            // 保存通关记录信息
            const recordData = DataCenterS.getData(this.userId, RecordData);
            recordData.saveShortestPassTime(GamesStartDefines.gameTheme, this.endCfgId, this.server_useTime);
            recordData.saveMinDeathAndBeHurtTimes(archiveData.beHurtTimes, archiveData.deathTimes);
        }
    }
