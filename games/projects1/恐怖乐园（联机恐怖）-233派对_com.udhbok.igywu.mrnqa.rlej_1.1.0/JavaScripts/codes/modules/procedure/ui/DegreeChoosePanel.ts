 ...
 * @LastEditors  : dal
 * @LastEditTime : 2024-05-23 18:26:28
 * @FilePath     : \1001_hall\JavaScripts\codes\modules\procedure\ui\DegreeChoosePanel.ts
 * @Description  : 
 */
...
export class DegreeChoosePanel extends Diffi_UI_Generate {
    ...
    private onDataReady() {
        WaitLoop.loop(() => { return DataCenterC.getData(ProcedureData) }).then((procedureData) => {
            ...
            procedureData.getPassedDegrees().forEach((degree: number) => { this._itemMap.get(degree).setImgVisible(); });

            // 不是A包根据情况显示锁
            if (!GameStart.IsAPackage) {
                this.degreeWithLock.forEach((degree) => {
                    if (!procedureData.getPassedDegrees().includes(degree - 1)) {
                        // 把锁带上
    ...
    protected onShow(style: StyleType, archiveId: number) {
        ...
        if (!GameStart.IsAPackage) {
            ...
            if (procedureData.getNewDegreeList().length != 0) {
                let newDegree = procedureData.getNewDegreeList()[0];
