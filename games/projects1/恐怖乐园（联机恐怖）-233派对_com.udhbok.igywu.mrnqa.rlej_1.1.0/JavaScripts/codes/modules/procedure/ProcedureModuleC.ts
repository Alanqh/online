 ...
 * @LastEditors  : dal
 * @LastEditTime : 2024-05-10 14:54:05
 * @FilePath     : \1001_hall\JavaScripts\codes\modules\procedure\ProcedureModuleC.ts
 * @Description  : 
 */
...
import ExpTipsHud from "./ui/ExpTipsHud";
...
export class ProcedureModuleC extends ModuleC<ProcedureModuleS, ProcedureData> {
    ...
    public savePassExp(useTimeSec: number, cfgId: number, degree: DegreeType, userId: string) {
        ...
        passTimeMinute = Math.max(1, passTimeMinute);
        ...
        this.getCurExp().then(v => {
            UIService.show(ExpTipsHud, exp, v);
            ModuleService.getModule(RouteModuleC).reqSetRouteData(userId, GameStart.GameTheme, [RouteDataType.TotalExp], [exp]);
        });
    }

    private async getCurExp() {
        const allGameRouteDataList = (await ModuleService.getModule(RouteModuleC).reqAllGameRouteData(Player.localPlayer.userId));
    ...
    private onPlayerWinByLocalEvent(guid: string, endingCfgId: string) {
        ...
        this.server.net_syncProcedureState(Player.localPlayer.userId, this.myScript.state, EndType.Success, Number(endingCfgId));
    ...
    public loadGame(degree: number, style: number, archiveID: number, isLoadByArchive: boolean = false) {
        // 已经有个游戏了，不能继续加载游戏了
        if (this.myScript.state == EmProcedureState.Loading || this.myScript.state == EmProcedureState.Start) {
            console.error(`DEBUG>>> MyType Error loadGame Error game instance is already created yet!`);
            return;
        }
