...
export default class UIGlobalRankPanel extends GlobalRankPanel_Generate {
    ...
    protected onAwake(): void {
        ...
        for (let index = 0; index < GlobalRankDefine.RankDataTypeList.length; index++) {
            let shiftBtn = this[`mBtn_Tab${index + 1}`];
            let txt = this[`textBlock${index + 1}`];
            let degreeType = GlobalRankDefine.RankDataTypeList[index];
    ...
    private initRankItems() {
        ...
        for (let i = 0; i < GlobalRankDefine.MaxRankItemViewLength; i++) {
