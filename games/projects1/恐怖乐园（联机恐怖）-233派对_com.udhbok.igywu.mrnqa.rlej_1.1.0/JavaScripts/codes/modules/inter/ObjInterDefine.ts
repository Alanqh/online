...
export class InterEvtData {
    ...
    @mw.Property({ displayName: "不读取存档" })
    public isnotArchive: boolean = false;
}
...
export class ObjInterDefine {
    public static isLoadingArchive: boolean = false;
    /**
     * 双端的事件
     */
     ...
     * 读存档的时候无视的事件
     */
    public static loadDisEvts = ["CGPlayer", "CameraLookAtEvt", "evt_playSound", "showTips"]
    ...
    public static dispatchClientByData(datas: InterEvtData[], defaultGuid: string) {
        ...
        if (SystemUtil.isClient()) {
            ...
            datas.forEach(element => {
                ...
                if (this.isLoadingArchive && this.loadDisEvts.includes(element.evtName)) {
                    return;
                }
                console.log("isReadingArchive" + this.isLoadingArchive)
                if (element.isnotArchive && this.isLoadingArchive) {
                    return;
                }
