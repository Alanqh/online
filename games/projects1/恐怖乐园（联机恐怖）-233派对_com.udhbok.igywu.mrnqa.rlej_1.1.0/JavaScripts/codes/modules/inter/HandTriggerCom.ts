...
export default class HandTriggerCom extends HandTrigger {
    ...
    private interFunc() {
        ...
        TimeScriptTrace.triggerDay(2, this.gameObject.gameObjectId);

        // 保存交互记录
        let type = this.evtDataArr.findIndex(v => { return v.evtName === "doorInter" }) === -1 ? 0 : 1;
        ModuleService.getModule(RecordModuleC).addHandCount(type);
    }
