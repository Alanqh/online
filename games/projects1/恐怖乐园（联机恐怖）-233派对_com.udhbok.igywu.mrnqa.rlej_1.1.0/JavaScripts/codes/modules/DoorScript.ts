...
export default class DoorScript extends LevelBase {
    ...
    @mw.Property({ displayName: "是否初始化后开启碰撞" })
    public isResetCo: boolean = true;
    ...
    protected onLevelStart(): void {
        if (SystemUtil.isClient()) {
            ...
            if (this.gameObject.parent) {
                ...
                if (trigger instanceof Trigger) {
                    ...
                    SpawnManager.asyncSpawn({ guid: "Anchor", replicates: false }).then(go => {
                        ...
                        this.isResetCo && go.setCollision(PropertyStatus.On, true);
                        this.resetData();
                    })
                }
            }
            ...
            this.addLocalListen("onBuildRelease", (uuid: string) => {
                if (this.gameObject["BuildingUUID"] != uuid) {
                    return;
                }
    ...
    public setCueSave() {
        ...
        this.save2Archive(2);
        this.isUseCue = true;
        ItemTraceHelper.instance.setItemTrace(this.unlockItemId, true);
    }

    public setCueUnSave() {
        if (!this.isUseCue || this.doorState == EDoorType.UnLock) {
            return;
        }
        this.save2Archive(0);
        this.isUseCue = false;
        ItemTraceHelper.instance.setItemTrace(this.unlockItemId, false);
    }
    ...
    onLoadData(data: ArchiveData): void {
        ...
        if (isUnlock == 1) {
            ...
            if (this.isOpen2CloseCollision) {
                this.isResetCo && this.gameObject.setCollision(CollisionStatus.QueryOnly, true);
            }
            !this.isResetCo && this.gameObject.setCollision(CollisionStatus.Off, true);
    ...
    private openCloseDoor(open: boolean) {
        ...
        if (open) {
            ...
            this.isResetCo && this.gameObject.setCollision(CollisionStatus.On, true);
        }
    }
