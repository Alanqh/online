...
export class ScenePropsRefreshMgr {
    ...
    private constructor() {
        GameConfig.RefreshPack.getAllElement().forEach(packCfg => {
            ...
            for (let index = 0; index < packCfg.numberList.length; index++) {
                ...
                data.items.push(new ItemData(cfg.id, weight, cfg.value));
                data.value = packCfg.value;
        ...
        GameConfig.RefreshPoint.getAllElement().forEach((ele) => {
            ...
            if (!ele.locat) {
                ele.locat = Vector.zero;
            }
            if (!ele.rot) {
                ele.rot = Vector.zero;
            }
            CommonUtils.roundVec(ele.locat, 2);
            CommonUtils.roundVec(ele.rot, 2);
            arr.addPoint(ele);
        })
    }
    ...
    public refreshSceneProps(diff: number) {
        let resArr = this.getDiffData(GameConfig.SceneRefreshProps.getAllElement().find(e => {
    ...
    public refreshNewProps(data: PoolScoreData) {
        let result: ClueSaveData[] = [];
        this._cluePointMap.forEach(e => {
            e.reset();
        })
        const cfgs = GameConfig.RefreshPack.getAllElement().filter(e => {
            return e.type == 2;
        })

        for (let index = 0; index < cfgs.length; index++) {
            let cfg = cfgs[index];
            const poolCfg = GameConfig.PrizePool.getElement(cfg.prizeId);
            if (!poolCfg) {
                console.error("[RefreshMgr]没有相应的prizeCfg" + cfg.lv + "是不是")
                continue;
            }
            let arr = this._cluePointMap.get(cfg.lv);
            if (!arr) {
                console.error("[RefreshMgr]没有相应的点位" + cfg.lv + "是不是")
                continue;
            }
            for (let j = 0; j < cfg.activenum; j++) {
                let point = arr.getRandomPoint();
                if (!point) {
                    console.error("[RefreshMgr]点位不够用了" + cfg.lv + "是不是")
                    continue;
                }
                let val = new ClueSaveData();
                let useMoney = data.scoreMap[cfg.lv] || 0;
                let id = 0;
                if (!poolCfg.isHasLimit) {
                    id = this.getRandomItem(cfg.lv);
                }
                else {
                    id = this.getRadnomItemWithMoney(cfg.lv, poolCfg.value - useMoney);
                }

                if (id == 0) {
                    console.error("[RefreshMgr]我寻思是不是钱不够花了")
                    continue;
                }
                const itemCfg = GameConfig.Item.getElement(id);
                data.scoreMap[cfg.lv] = useMoney + itemCfg.value;
                console.log("[RefreshMgr]costsome" + itemCfg.value + "res" + data.scoreMap[cfg.lv])
                val["lv"] = cfg.lv;
                val["val"] = this._itemMap.get(cfg.lv).value;
                val.assid = id;
        ...
        SystemUtil.isPIE && console.log("[RefreshMgr]福利池结果" + JSON.stringify(result));
        return result;
    }
    ...
    private insNewClues(wishPoints: RefreshData[]) {
        ...
        for (let index = 0; index < wishPoints.length; index++) {
            ...
            if (!arr) {
                console.error("[RefreshMgr]没有这个等级的点位 level:" + data.level + "优先检查表格");
                continue;
            }
            for (let j = 0; j < data.count; j++) {
                ...
                if (val.assid == 0) {
                    console.error("[RerefreshMgr]生成失败，检查一下表格\n生成失败，检查一下表格" + data.level)
                    continue;
                }
                val["lv"] = data.level;
                val["val"] = this._itemMap.get(data.level).value;
                val.loc = CommonUtils.vec2Arr(point.locat);
                val.rot = CommonUtils.vec2Arr(point.rot);
                result.push(val);
            }
    ...
    private getRadnomItemWithMoney(level: number, money: number): number {
        console.log("[RefreshMgr] 尝试刷新一个包，你现在有" + money)
        let data = this._itemMap.get(level);
        if (!data) {
            console.error("[RefreshMgr]生成失败，检查一下表格\n生成失败，检查一下表格" + level)
            return 0;
        ...
        let rate = Math.random() * newWeight;
