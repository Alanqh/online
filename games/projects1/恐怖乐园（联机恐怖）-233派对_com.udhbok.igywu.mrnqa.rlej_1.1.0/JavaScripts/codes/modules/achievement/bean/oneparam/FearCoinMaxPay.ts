/*
 * @Author       : dal
 * @Date         : 2024-05-08 17:51:51
 * @LastEditors  : dal
 * @LastEditTime : 2024-05-11 10:31:42
 * @FilePath     : \1001_hall\JavaScripts\codes\modules\achievement\bean\oneparam\FearCoinMaxPay.ts
 * @Description  : 
 */
...
export default class FearCoinMaxPay extends AchievementS {

    public getCurProgress(achievementId: number): string {
        const cfg = this.cfgArr.find(v => { return v.id === achievementId });
        return `${this.getPlayerRecordData().fearCoinInfo.maxPay}/${cfg.targetPara[0]}`;
    }
