/*
 * @Author       : dal
 * @Date         : 2024-05-08 17:50:00
 * @LastEditors  : dal
 * @LastEditTime : 2024-05-11 10:08:02
 * @FilePath     : \1001_hall\JavaScripts\codes\modules\achievement\bean\oneparam\FearCoinTotalPay.ts
 * @Description  : 
 */
...
export default class FearCoinTotalPay extends AchievementS {

    public getCurProgress(achievementId: number): string {
        const cfg = this.cfgArr.find(v => { return v.id === achievementId });
        return `${this.getPlayerRecordData().fearCoinInfo.totalPay}/${cfg.targetPara[0]}`;
    }
