/*
 * @Author       : dal
 * @Date         : 2024-05-09 09:10:52
 * @LastEditors  : dal
 * @LastEditTime : 2024-05-11 10:07:25
 * @FilePath     : \1001_hall\JavaScripts\codes\modules\achievement\bean\oneparam\FearCoinMaxHave.ts
 * @Description  : 
 */
import { AchievementS, registerAchievementSAndC } from "../Achievement";

@registerAchievementSAndC
export default class FearCoinMaxHave extends AchievementS {

    public getCurProgress(achievementId: number): string {
        const cfg = this.cfgArr.find(v => { return v.id === achievementId });
        return `${this.getPlayerRecordData().fearCoinInfo.maxHave}/${cfg.targetPara[0]}`;
    }
