...
import { PickItemTag, PickSaveData } from "../procedure/const/ClueDefine";
...
export default class ItemPicker extends LevelBase {
    ...
    protected async pickUpItem(playerId: number) {
        if (this.gameObject[PickSaveData]) {
            const data = this.gameObject[PickSaveData] as ClueSaveData
            this.itemId = data.assid;
            if (data["lv"]) {
                let degree = BoardHelper.GetTargetKeyValue(BoardKeys.Degree);
                GhostTraceHelper.uploadMGS("ts_action_resource_change", "玩家拾取组合包内道具上发", { getinfo_id: data["lv"], getinfo_num: data["val"], costinfo_type: Number(degree) });
            }
        }
