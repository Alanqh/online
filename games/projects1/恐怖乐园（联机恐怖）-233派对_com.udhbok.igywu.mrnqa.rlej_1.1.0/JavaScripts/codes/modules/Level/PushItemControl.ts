...
export default class PushItemControl extends LevelBase {
    ...
    onLoadData(data: ArchiveData): void {
        ...
        if (isUnlock == -1) {
            this.isUseCue = true;
            ItemTraceHelper.instance.setItemTrace(this.canPushItemCfgId[0], true);
        }
    }
    ...
    public setCueUnSave() {
        if (!this.isUseCue || this.isPushed) {
            return;
        }
        this.save2Archive(0);
        this.isUseCue = false;
        ItemTraceHelper.instance.setItemTrace(this.canPushItemCfgId[0], false);
    }
}