export class UIUtils {
     ...
     * @param ui ui对象
     * @param show 可见
     * @param interact 可交互，默认false自身不可交互
     */
    public static setUIVibility(ui: mw.Widget, show: boolean, interact: boolean = false): void {
        try {
            if (!ui) return;
            if (show) {
                ui.visibility = interact ? mw.SlateVisibility.Visible : mw.SlateVisibility.SelfHitTestInvisible;
            }
            else {
                ui.visibility = mw.SlateVisibility.Collapsed;
            }
        } catch (error) {
            // 报错不处理            
        }
