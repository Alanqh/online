...
import { EquipClsMap } from "./items/Item";
...
export default class EquipScript extends mw.Script {
    ...
    async onEquipDataChanged() {
        ...
        if (this._isSelfChar) {
            if (this.lastEquipData) {
                let cfg = GameConfig.Item.getElement(this.lastEquipData.cfgId);
                if (cfg.clazz == "InteractItem" || cfg.clazz == "ScreamKunItem") {
        ...
        if (this.equipData) {
            if (itemIns) itemIns["customData"] = this.equipData.customData;
    ...
    private async tryEquipGoToChar() {
        ...
        const ele = GameConfig.Item.getElement(this.equipData.cfgId);
        const eleRes = await BagDefine.getCfgEx(this.equipData.cfgId, this._char.player.userId);
        if (!eleRes.res) {
            if (this.screenGo) {
                this.screenGo?.destroy();
                this.screenGo = null;
        ...
        let newGo = await GameObject.asyncSpawn(eleRes.res);
    ...
    private async tryEquipGoToScreen() {
        if (this.screenGo && !this.screenGo.isDestroyed) {
            this.screenGo?.destroy();
            this.screenGo = null;
            this.useUpdate = false;
        }
        ...
        const ele = GameConfig.Item.getElement(this.equipData.cfgId);
        const eleRes = await BagDefine.getCfgEx(this.equipData.cfgId, Player.localPlayer.userId);
        if (!ele) return;
        if (!eleRes.res) {
            if (this.screenGo) {
                this.screenGo?.destroy();
