...
import { UIBuyFix } from "../../store/ui/UIBuyGiftBag";
...
export class HintUI extends Hint_UI_Generate {
    ...
    onStart() {
        ...
        this.btn_useHint.onClicked.add(() => {
            ...
            if (itemCount >= this._needCount) {
                ...
                if (this._shopCfg) {
                    UIService.show(UIBuyFix, this._shopCfg.id, this._needCount - itemCount, (res: boolean) => {
                        GhostTraceHelper.uploadMGS("ts_action_fuel_bag", "自己采集燃料", { fuel_bag: res ? 0 : 1 });
                    });
                }
                else {
        ...
        this.btn_close1.onClicked.add(() => {
            ItemTraceHelper.instance.hideTrace();
        })
        this.hideHint();
        this.hideClose();
    }
    ...
    showHint(order: number) {
        this.canvas_hint.visibility = SlateVisibility.SelfHitTestInvisible;
        this.canvas_close.zOrder = order;
    }

    hideHint() {
        this.canvas_hint.visibility = SlateVisibility.Collapsed;
    }

    showClose() {
        this.canvas_close.visibility = SlateVisibility.SelfHitTestInvisible;
    }

    hideClose() {
        this.canvas_close.visibility = SlateVisibility.Collapsed;
    }
    ...
    setNum(num: number) {
        ...
        if (num == 0) {
            ...
            this.text_num.text = `${itemCount}/${num}`
...
export class HintTraceUI extends GhostDistance_UI_Generate {
    ...
    onShow() {
        UIService.getUI(HintUI).showClose();
    }

    onHide() {
        UIService.getUI(HintUI).hideClose();
