...
export default class EmojiUI extends Pose_UI_Generate {
    ...
    onStart() {
        for (let i = 1; i <= 4; i++) {
            this[`btn_pose${i}`].onClicked.add(() => {
                ...
                //先上滚轮再想办法
                let startPos: Vector2 = new Vector2(1502 + this._offset.x, 80 + this._offset.y);
    ...
    onShow() {
        this._offset = UIService.getUI(MainUI).canvas_smile.position.clone();

        this.mrootcanvas.position = new Vector2(this._offset.x, this._offset.y);
